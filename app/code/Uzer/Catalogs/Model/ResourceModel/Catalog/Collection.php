...
namespace Uzer\Catalogs\Model\ResourceModel\Catalog;

use Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection;
use Uzer\Catalogs\Model\Catalog as Model;
...
class Collection extends AbstractCollection
{
     ...
     * @var string
     */
    protected $_eventPrefix = 'uzer_catalogs_collection';
    ...
    protected function _renderFiltersBefore()
    {
        foreach (array('store') as $key) {
            if ($this->getFilter($key)) {
                $this->getSelect()->join(
                    [$key . '_table' => $this->getTable('uzer_catalogs_' . $key)],
                    'main_table.entity_id = ' . $key . '_table.catalogs_id',
                    []
                )->group(
